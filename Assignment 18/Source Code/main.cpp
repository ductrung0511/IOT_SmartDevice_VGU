...
#include <vector>
std::vector<int> simulatedProximity = {25, 45, 35, 20, 15, 40, 10, 60, 30, 28};
unsigned long lastProximityCheck = 0;
const unsigned long proximityCheckInterval = 1000; // 1 second
...
void loop() {
  ...
  if (millis() - lastProximityCheck >= proximityCheckInterval) {
    lastProximityCheck = millis();

    int index = random(simulatedProximity.size());
    int currentProximity = simulatedProximity[index];

    Serial.print("Proximity: ");
    Serial.print(currentProximity);
    Serial.println(" cm");

    if (currentProximity < 30) {
      sendImage();
    }
  }
